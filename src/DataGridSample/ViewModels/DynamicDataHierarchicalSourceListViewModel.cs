...
{
    public class DynamicDataHierarchicalSourceListViewModel : ObservableObject, IDisposable
    {
        private const string NamePropertyPath = "Item.Name";
        private const string PricePropertyPath = "Item.Price";

        private readonly SourceList<HierarchicalStreamingItem> _source;
        ...
        public DynamicDataHierarchicalSourceListViewModel()
        {
            _source = new SourceList<HierarchicalStreamingItem>();
        ...
        private void ResetItems()
        {
            _source.Edit(list =>
            {
                ...
                var items = new List<HierarchicalStreamingItem>(_targetRootCount);
        ...
        private void ApplyUpdates()
        {
            ...
            _source.Edit(list =>
            {
                var additions = new List<HierarchicalStreamingItem>(batch);
                for (var i = 0; i < batch; i++)
                {
                    additions.Add(CreateRoot());
                }

                list.AddRange(additions);

                var removeCount = list.Count - _targetRootCount;
                if (removeCount > 0)
                {
